...
TcpServer::TcpServer(const in_port_t port, handler_function_t handler) : port(port), handler(handler) {}
...
ssize_t TcpServer::Client::loadData() { return recv(socket, buffer, buffer_size, 0); }
